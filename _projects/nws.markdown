...
If we apply $$f$$ to a more complex signal, $$y[n]$$, we encouter a phenomenon called _intermodulation distortion_, which produces frequencies at $$a\omega_1 \pm b\omega_2, \; \forall a,b \in \mathbb{Z}^+$$, for input frequencies $$\omega_1$$ and $$\omega_2$$.
...
And it turns out that we can get good results using _tiny_ MLPs to represent our shaping functions -- only 169 parameters each!
